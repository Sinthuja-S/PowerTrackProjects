...
{
    int remaining_data=encInfo->image_capacity-16-32-32-32-(encInfo->size_secret_file*8)+1;
    char str[remaining_data];
    fread(str,remaining_data,1,encInfo->fptr_src_image);
    fwrite(str,remaining_data,1,encInfo->fptr_stego_image);
    return e_success;
}
...
Status encode_secret_file_data(EncodeInfo *encInfo)
{
  ...
  if(getc(encInfo ->fptr_stego_image)==EOF)
  return e_success;
}
        ...
        if(check_capacity(encInfo)==e_success)
        {
            ...
            if(copy_bmp_header(encInfo -> fptr_src_image,encInfo -> fptr_stego_image)==e_success)
            {
                ...
                if(encode_magic_string(MAGIC_STRING,encInfo)==e_success)
                {
                    ...
                    {
                        printf("Encoded extn size successfully\n");
                        if(encode_secret_file_extn(encInfo->extn_secret_file,encInfo)==e_success)
                        {
                            ...
                            if(encode_secret_file_size(encInfo->size_secret_file,encInfo)==e_success)
                            {
                                ...
                                printf("Failed to encoded secret file size\n");
                                return e_failure;
                            }
                        }
                        else{
                        ...
                        printf("Failed to encode the extn size\n");
                        return e_failure;
                    }
                }
                else{
